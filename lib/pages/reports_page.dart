...
class _ReportsPageState extends State<ReportsPage> {
  ...
  List<Map<String, dynamic>> _filteredData = [];  // Add this
  bool _isLoading = false;

  final _searchController = TextEditingController();  // Add this
  ...
  Future<void> _loadReportData() async {
    ...
    try {
      ...
      setState(() {
        _studentsData = data;
        _filteredData = data;  // Add this line
      });
    } catch (e) {
      if (mounted) {
...
Future<void> _exportToExcel() async {
  try {
    ...
    sheet.appendRow([
      ...
      TextCellValue('FEE PAYMENT DATE'),  // NEW COLUMN
      TextCellValue('FINES STATUS'),
      TextCellValue('FINES PAYMENT DATE'),  // NEW COLUMN
    ]);
    ...
    for (var student in _studentsData) {
      ...
      sheet.appendRow([
        ...
        TextCellValue(student['fee_paid'] ? 'PAID' : student['outstanding_fee'] > 0 ? 'UNPAID' : 'N/A'),
        TextCellValue(feeDate),  // NEW: Fee payment date
        TextCellValue(student['fines_paid'] ? 'PAID' : student['outstanding_fines'] > 0 ? 'UNPAID' : 'N/A'),
        TextCellValue(finesDate),  // NEW: Fines payment date
      ]);
    ...
    if (student['fee_paid']) {
      var cell = sheet.cell(CellIndex.indexByColumnRow(columnIndex: 8, rowIndex: rowIndex));
      cell.cellStyle = CellStyle(backgroundColorHex: ExcelColor.green300);
    }
    
    // Fee date cell (column 9)
    if (student['fee_paid']) {
      var cell = sheet.cell(CellIndex.indexByColumnRow(columnIndex: 9, rowIndex: rowIndex));
      cell.cellStyle = CellStyle(backgroundColorHex: ExcelColor.green300);
    }
    
    // Fines status cell (column 10)
    if (student['fines_paid']) {
      var cell = sheet.cell(CellIndex.indexByColumnRow(columnIndex: 10, rowIndex: rowIndex));
      cell.cellStyle = CellStyle(backgroundColorHex: ExcelColor.green300);
    }
    
    // Fines date cell (column 11)
    if (student['fines_paid']) {
      var cell = sheet.cell(CellIndex.indexByColumnRow(columnIndex: 11, rowIndex: rowIndex));
      cell.cellStyle = CellStyle(backgroundColorHex: ExcelColor.green300);
    }
  }
    // Save file
                        ...
                        child: SingleChildScrollView(
                          child: DataTable(
                            ...
                            columns: const [
                              ...
                              DataColumn(label: Text('YEAR LEVEL', style: TextStyle(fontWeight: FontWeight.bold))),
                              DataColumn(label: Text('FEE AMOUNT', style: TextStyle(fontWeight: FontWeight.bold))),
                              DataColumn(label: Text('FINES AMOUNT', style: TextStyle(fontWeight: FontWeight.bold))),
                              DataColumn(label: Text('FEE STATUS', style: TextStyle(fontWeight: FontWeight.bold))),
                              DataColumn(label: Text('FINES STATUS', style: TextStyle(fontWeight: FontWeight.bold))),
                            ],
                            rows: _filteredData.map((student) {
                              return DataRow(
                                cells: [
                                  ...
                                  DataCell(Text('₱${student['outstanding_fee']?.toStringAsFixed(2) ?? '0.00'}')),
                                  DataCell(Text('₱${student['outstanding_fines']?.toStringAsFixed(2) ?? '0.00'}')),
                                  DataCell(
                                    Container(
                                      padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                                      decoration: BoxDecoration(
                                        color: student['fee_paid'] ? Colors.green[100] : null,
                                      ...
                                      child: Text(
                                        student['fee_paid'] 
                                            ...
                                            : student['outstanding_fee'] > 0 
                                                ...
                                                : 'N/A',
                                        style: TextStyle(
                                          color: student['fee_paid'] ? Colors.green[900] : Colors.black,
                                          fontWeight: student['fee_paid'] ? FontWeight.bold : FontWeight.normal,
                                        ),
                                      ),
                                    ),
                                  ),
                                  DataCell(
                                    Container(
                                      padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                                      decoration: BoxDecoration(
                                        color: student['fines_paid'] ? Colors.green[100] : null,
                                        borderRadius: BorderRadius.circular(4),
                                      ),
                                      child: Text(
                                        student['fines_paid'] 
                                            ...
                                            : student['outstanding_fines'] > 0 
                                                ...
                                                : 'N/A',
                                        style: TextStyle(
                                          color: student['fines_paid'] ? Colors.green[900] : Colors.black,
                                          fontWeight: student['fines_paid'] ? FontWeight.bold : FontWeight.normal,
                                        ),
                                      ),
                                    ),
                                  ),
